...
// ─── Tooltip styles ───────────────────────────────────────────────────────────
const tooltipStyle = {
  contentStyle: { background: '#0f1117', border: '1px solid #334155', borderRadius: 8, fontSize: 11 },
    ...
    <div className="mt-2">
      ...
      {open && (
        <div className="mt-2 space-y-2 pl-3 border-l border-amber-400/30">
          <div>
            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-0.5">Root Cause</p>
            <p className="text-xs text-slate-200 leading-relaxed">{alarm.rootCause}</p>
          </div>
          {alarm.impactAssessment && (
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-0.5">Impact Assessment</p>
              <p className="text-xs text-slate-200 leading-relaxed">{alarm.impactAssessment}</p>
            </div>
          )}
          {alarm.recommendedAction && (
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-0.5">Recommended Action</p>
              <p className="text-xs text-slate-200 leading-relaxed whitespace-pre-line">{alarm.recommendedAction}</p>
            </div>
          )}
        </div>
      )}
...
function AssetDetailPanel({ asset, onClose }: { asset: CrossingAsset; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="h-full w-[600px] bg-[#0f1117] border-l border-slate-700/50 overflow-y-auto shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#0f1117] border-b border-slate-700/50 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-sm font-bold text-white">{asset.streetName}</h2>
              <p className="text-xs text-slate-400">{asset.city}, {asset.province} · {asset.subdivision} Sub MP {asset.milepost} · DOT# {asset.dotNum}</p>
            </div>
            <button onClick={onClose} className="text-slate-400 hover:text-white mt-1"><XCircle size={18} /></button>
          </div>
          <div className="flex gap-1 mt-3">
            {(['overview', 'dau', 'wsdmm', 'alarms', 'events'] as const).map(t => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`px-3 py-1 text-[11px] font-medium rounded transition-colors ${tab === t ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200'}`}
              >
                {t === 'dau' ? 'DAU' : t === 'wsdmm' ? 'WSDMM' : t.charAt(0).toUpperCase() + t.slice(1)}
                {t === 'alarms' && asset.activeAlarms.length > 0 && (
                  <span className="ml-1 bg-red-500 text-white text-[9px] rounded-full px-1">{asset.activeAlarms.length}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {tab === 'overview' && (
            <>
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">SNOW Integration</p>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div><span className="text-slate-500">Asset ID:</span> <span className="text-slate-200 font-mono">{asset.snowAssetId}</span></div>
                  <div><span className="text-slate-500">Last Sync:</span> <span className="text-slate-200">{asset.snowLastSync}</span></div>
                  <div><span className="text-slate-500">Last Inspection:</span> <span className="text-slate-200">{asset.lastInspection}</span></div>
                  <div><span className="text-slate-500">Next Due:</span> <span className="text-slate-200">{asset.nextInspectionDue}</span></div>
                </div>
              </div>
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Maintenance Mode</p>
                  <button
                    onClick={() => setShowMaintenanceInput(v => !v)}
                    className={`flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded border transition-colors ${asset.maintenanceMode ? 'bg-blue-400/10 border-blue-400/30 text-blue-400 hover:bg-blue-400/20' : 'bg-slate-700/50 border-slate-600/50 text-slate-300 hover:bg-slate-700'}`}
                  >
                ...
                {asset.maintenanceMode ? (
                  <div className="space-y-1 text-xs">
                    <div><span className="text-slate-500">Set by:</span> <span className="text-slate-200">{asset.maintenanceModeSetBy}</span></div>
                    <div><span className="text-slate-500">Since:</span> <span className="text-slate-200">{asset.maintenanceModeSetAt}</span></div>
                    <div><span className="text-slate-500">Reason:</span> <span className="text-slate-200">{asset.maintenanceModeReason}</span></div>
                  ...
                  <p className="text-xs text-slate-400">Not in maintenance mode. Alarms and SNOW tickets are active.</p>
                )}
                {showMaintenanceInput && !asset.maintenanceMode && (
                  <div className="mt-2 space-y-2">
                    <textarea value={maintenanceNote} onChange={e => setMaintenanceNote(e.target.value)} placeholder="Enter reason for maintenance mode..." className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-xs text-slate-200 placeholder-slate-500 resize-none h-16" />
              ...
              {asset.crossingId === 'CX-002' && (
                <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-3">24h Voltage Trend (V1 Battery · V3 Gate East)</p>
              ...
              {asset.activeAlarms.length > 0 && (
                <div>
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Active Alarms ({asset.activeAlarms.length})</p>
                  {asset.activeAlarms.map(alarm => (
                    <div key={alarm.alarmId} className={`rounded-lg p-3 border mb-2 ${alarm.severity === 'CRITICAL' ? 'bg-red-400/5 border-red-400/20' : 'bg-amber-400/5 border-amber-400/20'}`}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-mono text-slate-400">{alarm.alarmCode}</span>
                      </div>
                      <p className="text-xs text-slate-200 leading-relaxed">{alarm.description}</p>
                      {alarm.snowTicketId && (
                      ...
                      <p className="text-[10px] text-slate-500 mt-2">{alarm.timestamp}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {tab === 'dau' && (
            <div className="space-y-3">
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">DAU Identity</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  {[['Serial #', asset.dau.serialNumber], ['IP Address', asset.dau.ipAddress], ['MAC Address', asset.dau.macAddress], ['Part #', asset.dau.partNumber], ['SW Version', asset.dau.softwareVersion], ['HW Version', asset.dau.hardwareVersion], ['OS', asset.dau.osVersion], ['Uptime', asset.dau.uptime]].map(([k, v]) => (
                    <div key={k}><span className="text-slate-500">{k}:</span> <span className="text-slate-200 font-mono text-[11px]">{v}</span></div>
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Voltages</p>
                ...
                <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">System Resources</p>
                    ...
                    return (
                      <div key={label as string} className="mb-1.5">
                        <div className="flex justify-between text-[11px] mb-0.5">
                          ...
                          <span className={warn ? 'text-amber-400 font-medium' : 'text-slate-200'}>{display as string}</span>
                        </div>
                        {p !== null && <div className="h-1 bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full ${warn ? 'bg-amber-400' : 'bg-emerald-400'}`} style={{ width: `${p}%` }} /></div>}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Digital Inputs (Circuit States)</p>
                <div className="grid grid-cols-3 gap-1.5">
                  {[['DI1 Island', asset.dau.di1], ['DI2 Approach E', asset.dau.di2], ['DI3 Approach W', asset.dau.di3], ['DI4 Gate Down E', asset.dau.di4], ['DI5 Gate Down W', asset.dau.di5], ['DI6 Bell', asset.dau.di6], ['DI7 Flash', asset.dau.di7], ['DI8 Power Fail', asset.dau.di8], ['DI9 Standby Pwr', asset.dau.di9], ['DI10 Maint SW', asset.dau.di10], ['DI11 Reserved', asset.dau.di11], ['DI12 Reserved', asset.dau.di12]].map(([label, val]) => (
                    <div key={label as string} className={`flex items-center gap-1.5 text-[10px] px-2 py-1 rounded border ${val ? 'bg-emerald-400/10 border-emerald-400/20 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
          ...
          {tab === 'wsdmm' && (
            <div className="space-y-3">
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">WSDMM Identity</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
              ...
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Docker Images</p>
                <p className="text-xs font-mono text-slate-200">{asset.wsdmm.dockerImages}</p>
              </div>
              <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Remote Configuration</p>
                <div className="flex gap-2">
                  <button className="text-xs bg-slate-700/50 border border-slate-600/50 text-slate-300 px-3 py-1.5 rounded hover:bg-slate-700 transition-colors flex items-center gap-1.5"><FileText size={12} /> Download Config</button>
          ...
          {tab === 'alarms' && (
            <div className="space-y-2">
              {asset.activeAlarms.length === 0 ? (
                <div className="text-center py-8"><CheckCircle size={32} className="text-emerald-400 mx-auto mb-2" /><p className="text-sm text-slate-400">No active alarms</p></div>
              ) : asset.activeAlarms.map(alarm => (
                <div key={alarm.alarmId} className={`rounded-lg p-3 border ${alarm.severity === 'CRITICAL' ? 'bg-red-400/5 border-red-400/20' : 'bg-amber-400/5 border-amber-400/20'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${severityBg(alarm.severity)}`}>{alarm.severity}</span>
                    <span className="text-[10px] font-mono text-slate-400">{alarm.alarmCode}</span>
                    <span className="text-[10px] text-slate-500 ml-auto">{alarm.timestamp}</span>
                  </div>
                  <p className="text-xs text-slate-200 leading-relaxed mb-1">{alarm.description}</p>
                  ...
                  <div className="flex gap-2 mt-2">
                    <button className="text-[10px] bg-slate-700/50 border border-slate-600/50 text-slate-300 px-2 py-1 rounded hover:bg-slate-700 transition-colors">Manually Clear</button>
          ...
          {tab === 'events' && (
            <div className="space-y-2">
              {asset.recentEvents.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-8">No recent events</p>
              ) : asset.recentEvents.map(evt => (
                <div key={evt.eventId} className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/40">
                  <div className="flex items-center gap-2 mb-1">
                    {eventTypeIcon(evt.eventType)}
                    <span className="text-[10px] font-mono text-slate-400">{evt.eventType}</span>
                    <span className="text-[10px] text-slate-500 ml-auto">{evt.timestamp}</span>
                  </div>
                  <p className="text-xs text-slate-200 leading-relaxed">{evt.description}</p>
                  {evt.details && Object.keys(evt.details).length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1">
                      {Object.entries(evt.details).map(([k, v]) => (
                        <span key={k} className="text-[10px] font-mono bg-slate-900 border border-slate-700 px-1.5 py-0.5 rounded text-slate-300">{k}: {String(v)}</span>
                      ))}
                    </div>
                  )}
                </div>
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-[#0a0c10] text-slate-200">
        ...
        {/* Page Header */}
        <div className="border-b border-slate-800/60 bg-[#0d0f14] px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">DAU · WSDMM · OWL Agent · SNOW Integration — Last refresh: {lastRefresh}</p>
            </div>
            <button className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-slate-200 border border-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">
          ...
          <div className="grid grid-cols-7 gap-3 mb-4">
            {[
              { label: 'Total', value: summary.total, color: 'text-slate-200', icon: <Radio size={13} /> },
          ...
          <div className="flex gap-1">
            {(['assets', 'alarms', 'events', 'analytics'] as const).map(v => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`px-4 py-1.5 text-xs font-medium rounded-lg transition-colors ${view === v ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}
              >
        ...
        {/* Filters */}
        <div className="px-6 py-3 border-b border-slate-800/40 flex items-center gap-3 flex-wrap bg-[#0d0f14]">
          <div className="relative">
            <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search crossings..." className="bg-slate-800 border border-slate-700 rounded-lg pl-7 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 w-48 focus:outline-none focus:border-cyan-500/50" />
          </div>
          <select value={subFilter} onChange={e => setSubFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none">
            {subdivisions.map(s => <option key={s}>{s}</option>)}
          </select>
          {view === 'assets' && (
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none">
              {['All', 'ONLINE', 'DEGRADED', 'OFFLINE', 'MAINTENANCE'].map(s => <option key={s}>{s}</option>)}
            </select>
          )}
          {view === 'alarms' && (
            <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none">
                ...
                <thead>
                  <tr className="border-b border-slate-700/60">
                    {['Crossing', 'Subdivision', 'MP', 'DOT #', 'Status', 'Alarms', 'DAU SW', 'WSDMM', 'Last Heartbeat'].map(h => (
                      <th key={h} className="py-2 px-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredAssets.map(asset => {
                    ...
                    return (
                      <tr key={asset.crossingId} className="border-b border-slate-800/60 hover:bg-slate-800/30 cursor-pointer transition-colors" onClick={() => setSelectedAsset(asset)}>
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-xs font-medium text-slate-200">{asset.streetName}</p>
                              <p className="text-[10px] text-slate-500">{asset.city}, {asset.province}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-2.5 px-3 text-xs text-slate-300">{asset.subdivision}</td>
                        <td className="py-2.5 px-3 text-xs font-mono text-slate-400">{asset.milepost}</td>
                        <td className="py-2.5 px-3 text-xs font-mono text-slate-400">{asset.dotNum}</td>
                        <td className="py-2.5 px-3">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${statusBg(asset.maintenanceMode ? 'MAINTENANCE' : asset.status)}`}>
                        ...
                        <td className="py-2.5 px-3 text-[10px] text-slate-400">{asset.dau.softwareVersion}</td>
                        <td className="py-2.5 px-3 text-[10px] text-slate-400">{asset.wsdmm.wsdmmImageVersion}</td>
                        <td className="py-2.5 px-3 text-[10px] text-slate-500">{asset.dau.lastHeartbeat}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          {view === 'alarms' && (
            <div className="space-y-2">
              {filteredAlarms.length === 0 ? (
                <div className="text-center py-12"><CheckCircle size={40} className="text-emerald-400 mx-auto mb-3" /><p className="text-sm text-slate-400">No active alarms matching filters</p></div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span className="text-[10px] font-mono text-slate-400">{alarm.alarmCode}</span>
                          <span className="text-[10px] text-slate-400">{alarm.deviceType}</span>
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-[10px] text-slate-500">{alarm.timestamp}</p>
                        <div className="flex gap-1.5 mt-2 justify-end">
                          <button className="text-[10px] bg-slate-700/50 border border-slate-600/50 text-slate-300 px-2 py-1 rounded hover:bg-slate-700 transition-colors">Clear</button>
          ...
          {view === 'events' && (
            <div className="space-y-1.5">
              {filteredEvents.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-12">No events matching filters</p>
              ) : filteredEvents.map(evt => (
                <div key={evt.eventId} className="bg-slate-800/30 rounded-lg px-4 py-3 border border-slate-700/30 flex items-start gap-3 hover:bg-slate-800/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      ...
                      <span className="text-[10px] text-slate-500">·</span>
                      <span className="text-[10px] text-slate-500">{evt.subdivision} Sub MP {evt.milepost}</span>
                      <span className="text-[10px] text-slate-500">·</span>
                      <span className="text-[10px] font-medium text-slate-400">{evt.deviceType}</span>
                    </div>
                    <p className="text-xs text-slate-200 leading-relaxed">{evt.description}</p>
                    {evt.details && Object.keys(evt.details).length > 0 && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        {Object.entries(evt.details).map(([k, v]) => (
                          <span key={k} className="text-[10px] font-mono bg-slate-900 border border-slate-700 px-1.5 py-0.5 rounded text-slate-400">{k}: {String(v)}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <p className="text-[10px] text-slate-500 shrink-0">{evt.timestamp}</p>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/40">
                  <p className="text-xs font-semibold text-slate-300 mb-3">Crossing Status Distribution</p>
                ...
                <div className="col-span-2 bg-slate-800/40 rounded-xl p-4 border border-slate-700/40">
                  <p className="text-xs font-semibold text-slate-300 mb-3">Active Alarms by Subdivision</p>
                  <ResponsiveContainer width="100%" height={180}>
              ...
              {/* Heartbeat Timeline */}
              <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/40">
                <p className="text-xs font-semibold text-slate-300 mb-3">Crossing Availability — Last 12 Hours</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/40">
                  <p className="text-xs font-semibold text-slate-300 mb-3">DAU Software Versions</p>
                  {(() => {
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/40">
                  <p className="text-xs font-semibold text-slate-300 mb-3">WSDMM Image Versions</p>
                  {(() => {
                    const versionCounts: Record<string, number> = {};
